(function(scope) {
    // Number of items instantiate beyond current view in the scroll direction.
    var RUNWAY_ITEMS = 50;

    // Number of items instantiate beyond current vew in the opposite direction.
    ...
    scope.InfiniteScrollSource = function() {};

    scope.InfiniteScrollSource.prototype = {
        /**
         * Fetch more items from data source. This should try to fetch at least
         ...
         * @return {Promise(Array<Object>)} return a promise which will be resolved
         ...
         * @return {Element} A tombstone element to be displayed when item data is
         *     not yet available for the scrolled position.
         */
        createTombstone: function() {},

        /**
         * Render an item, re-useing the provided item div if passed in.
         ...
         *    element which should be recycled for the new item display.
    ...
    /**
     *  Constructed an infinite scroll.
     *  @param {Element} scroller The scrollable element to use as the infinte 
     *      scroll region
     *  @param {InfiniteScrollSource} source A provider of the content to be 
    ...
    scope.InfiniteScroller = function(scroller, source) {
        ...
        // Create an element to force scroller to allow scrolling to certain point
        ...
        this.scrollRunway_.textContext = '';
    ...
    scope.InfiniteScroller.prototype = {
        ...
        onScroll_: function() {
            ...
            if (delta < 0) {
                this.fill(this.anchorItem.index - RUNWAY_ITEMS, lastScreenItem.index + RUNWAY_ITEMS_OPPSITE);
            } else {
                this.fill(this.anchorItem.index - RUNWAY_ITEMS_OPPSITE, lastScreenItem.index + RUNWAY_ITEMS);
            }
        },
        /**
         * Calculates the item that should be anchored after scrolling by delta from
         * the initial anchored item.
         * @param {{index: number, offset: number}} initialAnchor The inital position
         ...
         * @return {{index: number, offset: number}} Return the new item and offset
        ...
        /**
         * Create or returns an existing tombstone ready to be reused.
        ...
        /**
         *  Attaches content to scroller and updates  the scroll position if
         *  necessary
         */
        attachContent: function() {
            ...
            for (i = this.firstAttachedItem_; i < this.lastAttachedItem_; i++) {
                while (this.items_.length <= i)
                    this.addItem_();
                if (this.items_[i].node) {
                    // if it's a tombstone but we have date, replace it.
                    if (this.items_[i].node.classList.contains('tombstone') &&
                        this.items_[i].data) {
                        // TODO: probably best to move items on top of tombstone and fade them in instead
         ...
         *  attachedContent to update the displayed content.
